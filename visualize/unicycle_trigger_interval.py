...
def unicycle_trigger_interval_vis(file_Lfh_traj, file_Lfh_num_traj, file_tau_traj, file_tau_num_traj, file_xvel_traj):
    Lfh_traj = np.loadtxt(file_Lfh_traj)
    Lfh_num_traj = np.loadtxt(file_Lfh_num_traj)
    tau_traj = np.loadtxt(file_tau_traj)
    tau_num_traj = np.loadtxt(file_tau_num_traj)
...
if '__main__' == __name__:
    ...
    file_Lfh_num_traj = osp.join(events_dir, "Lfh_num.np.txt")
    file_tau_traj = osp.join(events_dir, "tau.np.txt")
    file_tau_num_traj = osp.join(events_dir, "tau_num.np.txt")
    file_xvel_traj = osp.join(events_dir, "xvel.np.txt")
    data_files = (file_Lfh_traj, file_Lfh_num_traj, file_tau_traj, file_tau_num_traj, file_xvel_traj)
